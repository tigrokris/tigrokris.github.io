...
gulp.task("style", function () {      /*Описание таска*/
    gulp.src("less/style.less")
        ...
        .pipe(server.stream());
    /*Команда перезагрузки сервера в браузере*/
});
...
gulp.task("symbols", function () {
    return gulp.src("build/img/icons/*.svg")
        ...
        .pipe(svgstore({
            /*Вырезает из SVG-файлов лишнюю инф-цию*/
            inLineSvg: true
        }))
        .pipe(rename("sprite.svg")) /*нужно переименовать, так как мы не снаем имя спрайта*/
        .pipe(gulp.dest("build/img"));
});
